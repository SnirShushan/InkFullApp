...
class SSP {
     ...
     *
     */
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '', $SQLForTotalRecordsCount ='' )
    {
        $bindings = array();
       ...
       $having
        ...
        if($SQLForTotalRecordsCount == ''){
            ...
            $resTotalLength = SSP::sql_exec( $db,
                "SELECT COUNT(m.`{$primaryKey}`)
                 FROM ($SQLForTotalRecordsCount) as m LIMIT 1"
            );
            $recordsTotal = $resTotalLength[0][0];
        }
    ...
    static function simpleCustom ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '', $tableFromSql='' )
    {
        $bindings = array();
        $db = SSP::sql_connect( $sql_details );
       ...
       $having
        ...
        $resTotalLength = SSP::sql_exec( $db,
            "SELECT COUNT(m.`{$primaryKey}`)
             FROM  ($tableFromSql) as m LIMIT 1"
        );
        $recordsTotal = $resTotalLength[0][0];
