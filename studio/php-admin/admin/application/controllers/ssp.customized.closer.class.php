...
class SSP {
    ...
    public static $my_command = array();
    public static $sqlHistory = array();

    public static $allow_sql_in_table = false;


    public static function AllowSQLInTable()
    {
        self::$allow_sql_in_table = true;
    }
    ...
    static function order ( $request, $columns, $isJoin = false )
    {
		...
		if ( isset($request['order']) && is_array($request['order'] ?? null) && count($request['order']) ) {
            $orderBy = array();
     ...
     *
     */
    static function simple ( $request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '',$command=array())
    {
        self::$my_command = $command;

        //Start SQL Query
        $table_ = $table;
        if( self::$allow_sql_in_table ){
            $isSQLQuery = SSP::startsWith(trim($table_),"(");
       ...
       $having
        ...
        $resTotalLength = false;
        if( self::$allow_sql_in_table == False ){
    ...
    {
        try {
            $port = !empty($sql_details['port']) ? (int) $sql_details['port'] : 3306;
            $db = @new PDO(
                "mysql:host={$sql_details['host']};port={$port};dbname={$sql_details['db']};charset=utf8mb4",
                $sql_details['user'],
                $sql_details['pass'],
                array( PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION )
            );
            $db->query("SET NAMES 'utf8mb4'");//mb4
        }
    ...
    static function sql_exec ( $db, $bindings, $sql=null )
    {
        ...
        $pre_sql = "";
        if(count(self::$my_command)>0){
            if(isset($my_command['pre_sql'])){
                $pre_sql = $my_command['pre_sql'];
            }
        }

        $stmt = $db->prepare( $pre_sql.$sql );
        ...
        try {
            ...
            $lastQuery = self::$sqlHistory[count(self::$sqlHistory) - 1];
            /* echo '<pre>'; 
            print_r($lastQuery);
            echo '</pre>'; 
            exit;  */
