...
$corr_us = 'vLuaEXWYjZMxZZZpYSpY';
$corr_pass = 'vLuaEXWYjZMxZZZpYSpY';

if(!isset($_REQUEST['user']) || (isset($_REQUEST['user']) && $_REQUEST['user'] != $corr_us))
{
    die();
}
if(!isset($_REQUEST['pass']) || (isset($_REQUEST['pass']) && $_REQUEST['pass'] != $corr_pass))
{
    die();
}
...
{
    $json_data = file_get_contents(__DIR__."/errors/app_log/".$filename);
}
else{
    $log_date = real_escape($filename);
    // $res=file_get_contents(__DIR__."/errors/app_log/".$filename);
    $sql = "SELECT log_data FROM tbl_app_log WHERE log_date = '" . $_REQUEST['file'] . "'";
    $result = $db->query($sql);
    
    if ($result && $result->num_rows() > 0) {
        $json_data = $result->row()->log_data;
    }
}
