...
if ($check) {
    ...
    if ($row == false) {
        ...
        $sql_noti="SELECT 
            ...
            FROM tbl_notifications n JOIN
                tbl_customer c ON n.uid = c.id AND c.is_delete = '0'
                LEFT JOIN (
                    SELECT p1.*
                    FROM tbl_post p1
                    JOIN (
                        SELECT id, uid, MAX(id) AS max_id
                        FROM tbl_post
                        GROUP BY id, uid
                    ) p2 ON p1.id = p2.max_id
                ) pi ON pi.id = n.pid AND pi.uid = n.uid
            WHERE n.me='".$post['uid']."' AND n.uid = c.id AND c.is_delete = '0'
            ".$extraWhere." ORDER BY noti_date DESC ".$limit;
        $data['query'] = $sql_noti;
        $result=$db->query($sql_noti);
        $noti_list = $result->result_array() ?? [];
