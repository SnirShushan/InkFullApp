...
## What we saved
- Full MySQL dump: `data/inkisrael_app.sql` (imported locally as `inkisrael_app`)
...
| Piece | Service | Why |
|-------|---------|-----|
| API | Node/Express in `app/api/` | Simple for Flutter team, replaces PHP |
| Database | Railway MySQL | Same SQL dump, cheap managed MySQL |
| Images | Cloudflare R2 | Cheap object storage, no egress fees |

Old PHP under `studio/php-admin/` is **legacy only** — use it to recover data, not for new production.

## Your next 15 minutes (accounts)
1. **Cloudflare** → R2 → create bucket `ink-media` → enable public access → copy public base URL  
2. Create R2 API token (Object Read & Write) → put into `app/api/.env` (`R2_*`)  
3. Run: `node tools/backup/upload_to_r2.mjs`  
4. **Railway** → New project → MySQL → import `inkisrael_app.sql` → put DB_* into `app/api/.env`  
5. Deploy `app/api/` on Railway  
